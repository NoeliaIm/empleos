package com.noeliaiglesias.empleos.controller;

import java.util.Date;
import java.util.LinkedList;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.GetMapping;

import com.noeliaiglesias.empleos.model.Vacante;
import com.noeliaiglesias.empleos.service.IVacanteService;

@Controller
public class HomeController {
	
	@Autowired
	private IVacanteService vacanteService;

	@GetMapping("/")
	public String  mostrarHome(Model model) {
		//añadir al modelo un dato, que se inserta en tiempo de ejecución
		
		String nombre= "Auxiliar de Enfermería";
		Date fechaPub= new Date();
		double salario= 9000;
		boolean vigente= true;
		
		model.addAttribute("nombre",nombre);
		model.addAttribute("fecha", fechaPub);
		model.addAttribute("salario", salario);
		model.addAttribute("vigente", vigente);
		return "home";
	}
	
	
	@GetMapping("/listado")
	public String mostrarListado(Model model) {
		List<String> lista= new LinkedList<String>();
		lista.add("Ingeniero de sistemas");
		lista.add("Axiliar administrativo");
		lista.add("Dependiente");
		lista.add("Arquitecto");
		model.addAttribute("listado", lista);
		return "listado";
	}

	@GetMapping("/mostrarDetalle")
	public String mostrarDetalle(Model model) {
		Vacante vacante = new Vacante();
		vacante.setDescripcion("Labores de mantenimiento del servidor");
		vacante.setFecha(new Date());
		vacante.setId(1);
		vacante.setNombre("Ingeniero de sistemas");
		vacante.setSalario(1000.0);
		model.addAttribute("vacante", vacante);
		return "mostrarDetalle";
	}



	@GetMapping("/tablaVacantes")
	public String mostrarTabla(Model model) {
		List<Vacante> lista = vacanteService.buscarTodas();
		model.addAttribute("tablaVacantes", lista);
		return "tablaVacantes";
	}


}
